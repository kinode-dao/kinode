...
impl RotaryEmbedding {
    fn new(dtype: DType, cfg: &Config, dev: &Device) -> Result<Self> {
        ...
        let t = Tensor::arange(0.0, max_seq_len as f32, dev)?
            .to_dtype(dtype)?
...
impl Attention {
    ...
    fn get_kv_cache(&self) -> (Tensor, Tensor) {
        // We can unwrap here because we always set the kv_cache in the forward pass
        self.kv_cache.clone().unwrap()
    }

    fn set_kv_cache(&mut self, kv_cache: (Tensor, Tensor)) {
...
impl OriginModel {
    ...
    pub fn set_kv_caches(&mut self, kv_caches: Vec<(Tensor, Tensor)>) {
        for (layer, kv_cache) in self.layers.iter_mut().zip(kv_caches) {
            layer.set_kv_cache(kv_cache);
        }
    }

    pub fn forward(&mut self, input: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        let (b_size, seq_len) = input.dims2()?;
...
impl LinkModel {
    ...
    pub fn forward(&mut self, input: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        let (b_size, seq_len, _) = input.dims3()?;
        let attention_mask = if seq_len <= 1 {
            None
        } else {
            let mask = prepare_decoder_attention_mask(
        ...
        let mut xs = self.layers[0].forward(input, attention_mask.as_ref(), seqlen_offset)?;
...
impl EndModel {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let norm = RmsNorm::new(cfg.hidden_size, cfg.rms_norm_eps, vb_m.pp("norm")).ok();
        let lm_head = linear_no_bias(cfg.hidden_size, cfg.vocab_size, vb.pp("lm_head")).ok();
        Ok(Self {
            layers,
            norm,
            lm_head,
